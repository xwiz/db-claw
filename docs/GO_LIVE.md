...
uv run python scripts/generate_semsql_downloads_manifest.py \
  ...
  --base-url https://github.com/xwiz/db-claw/releases/download/v0.1.0-alpha.2 \
...
`python scripts/rehearse_release_packages.py --version 0.1.0-alpha.2`.
...
```bash
pnpm dlx --package @semsql/cli@0.1.0-alpha.2 \
  --package @semsql/extractor-cli@0.1.0-alpha.2 \
  semsql extract . \
  --framework laravel \
  --db-url "$DATABASE_URL" \
  --no-sample-values \
...
pnpm dlx @semsql/cli@0.1.0-alpha.2 doctor --graph storage/semsql/app.semsql
pnpm dlx @semsql/cli@0.1.0-alpha.2 query --graph storage/semsql/app.semsql "count active users"
```
