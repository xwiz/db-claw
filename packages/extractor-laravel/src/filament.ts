 ...
 * but require a real PHP AST walker. Tree-sitter-php integration lands in
 * v0.5; the regex-based v0.2 cut here covers the >80 % case in shipping
 * Filament apps.
 *
...
async function scanResourceFile(
    ...
    if (entity === null || entity === undefined) {
        result.skipped.push({ file, reason: `cannot derive canonical entity from $model = ${model}` });
        return;
    }
    ...
    pushIfPresent(result, canonical, props.singularLabel, file, props.singularLine, "modelLabel");
    pushIfPresent(result, canonical, props.pluralLabel, file, props.pluralLine, "pluralModelLabel");
    pushIfPresent(result, canonical, props.navLabel, file, props.navLine, "navigationLabel");
...
// i18n key and are deferred to v0.5 tree-sitter-php.
