 ...
 *    skipped and logged for the PHP AST path.
 *
...
async function scanLocaleDir(
	...
	for (const entry of entries) {
		...
		if (stat.isDirectory()) {
			...
			await scanPhpFile(
				full,
				groupPrefixFor(localeRoot, full, ".php"),
				locale,
				result,
			);
			...
			await scanJsonFile(
				full,
				groupPrefixFor(localeRoot, full, ".json"),
				locale,
				result,
			);
		}
...
// `skipped`. tree-sitter-php handles the full-coverage path.
// ---------------------------------------------------------------------------
