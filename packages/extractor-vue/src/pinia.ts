 ...
 *
 * The walker is regex-driven for v0.5; the v1.0 vue/compiler-sfc
...
export interface PiniaScanResult {
     ...
     *  in v1.0 to promote bare-ref `v-model="email"` to the qualified
     *  `entity.field` form. */
 ...
 */
export function extractStoreFields(body: string, isSetupForm: boolean): string[] {
    if (isSetupForm) {
...
function findMatchingClose(text: string, openIdx: number, openChar: string): number {
...
function emitStore(
    ...
    const indexEntry = result.entityIndex.get(canonicalEntity) ?? new Set<string>();
