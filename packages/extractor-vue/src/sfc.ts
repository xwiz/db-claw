 ...
 *
 * The walker uses `@vue/compiler-sfc` AST instead of regex. Handles:
 *
 ...
 * these against parent SFCs for end-to-end field resolution.
 *
...
function childrenOf(node: RootNode | TemplateChildNode): TemplateChildNode[] {
	...
	const anyNode = node as {
		...
		branches?: { children: TemplateChildNode[] }[];
	};
	if (anyNode.branches) {
 ...
 * The AST walker resolves when it can introspect the script
 * setup block.
 */
export function promoteBareRef(
	...
	// refs (`form.email.value`) are skipped unless they can be bound safely.
