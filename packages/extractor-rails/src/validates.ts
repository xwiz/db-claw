 ...
 *
 * Deliberately skipped by this validation reader:
 *
...
export function extractInclusionValidators(text: string): InclusionDecl[] {
	...
	const collect = (rx: RegExp, parser: (raw: string) => string[]): void => {
...
function sliceOriginalGroup(
	...
	const startInStripped = (match.index ?? 0) + match[0].indexOf(captured);
