 ...
 * `Person` → `people` is NOT special-cased; the inflector handles
 * the common pluralisation case, while irregulars need explicit mapping).
 *
 * Deliberately skipped by this enum reader:
 *
...
const TABLE_NAME_RX = /\bself\.table_name\s*=\s*(['"])((?:\\.|(?!\1).)*)\1/;
 ...
 * those skip the override and fall back to the inflector.
 *
 ...
 *   enum :status, [:draft, :published]
 */
const ENUM_HASH_KW_RX = /\benum\s+([A-Za-z_]\w*)\s*:\s*\{([^{}]*)\}/g;
const ENUM_ARR_KW_RX = /\benum\s+([A-Za-z_]\w*)\s*:\s*\[([^\[\]]*)\]/g;
const ENUM_HASH_POS_RX = /\benum\s+:([A-Za-z_]\w*)\s*,\s*\{([^{}]*)\}/g;
const ENUM_ARR_POS_RX = /\benum\s+:([A-Za-z_]\w*)\s*,\s*\[([^\[\]]*)\]/g;
 ...
 * (person → people, child → children) need explicit mapping; they
 ...
 * a full AST walker can distinguish via AST node type;
 * this reader takes the conservative path and ignores all string content.
 */
