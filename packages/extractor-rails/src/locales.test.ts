...
`,
		...
		const balance = r.fragments.find(
			(f) =>
				f.canonical.kind === "field" && f.canonical.field === "user.balance",
		);
...
`,
		...
		const roleFrag = r.fragments.find(
			(f) => f.canonical.kind === "field" && f.canonical.field === "role.name",
		);
...
`,
		...
		expect(r.index.get("activerecord.attributes.user.email")?.label).toBe(
			"Email Address",
		);
		expect(r.index.get("helpers.label.user.email")?.label).toBe("Your email");
...
`,
		...
		expect(r.index.get("views.pages.home.subtitle")?.label).toBe("Welcome");
	});
});
