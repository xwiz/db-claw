...
describe("scanDrizzleSchemas", () => {
	...
	it("emits ORM-layer field fragments with prettified labels", async () => {
		...
		expect(
			r.fragments.some(
				(f) => f.term === "users" && f.canonical.kind === "entity",
			),
		).toBe(true);
	...
	it("scans every documented schema location", async () => {
		...
		expect(r.fragments.length).toBe(18);
	});

	it("scans monorepo package schema folders", async () => {
		await write("packages/db/src/schema/users.ts", SAMPLE_SCHEMA);
		await write(
			"packages/db/src/schema-mysql/users.ts",
			SAMPLE_SCHEMA.replaceAll("pgTable", "mysqlTable"),
		);

		const r = await scanDrizzleSchemas(tmp);

		expect(r.fragments.length).toBe(12);
		expect(
			r.fragments.some(
				(f) =>
					f.canonical.kind === "field" &&
					f.canonical.field === "users.created_at" &&
					f.locator.file.includes("packages"),
			),
		).toBe(true);
	});
