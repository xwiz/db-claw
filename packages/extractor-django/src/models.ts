 ...
 *
 * Deliberately out of scope for this models walker:
 *
 ...
 */
export async function scanDjangoModels(
	root: string,
): Promise<DjangoModelsScanResult> {
	const result: DjangoModelsScanResult = { fragments: [], skipped: [] };

	// Three-phase orchestration:
	//
	// Parse every models.py, build per-file ChoicesClass +
...
function collectImports(root: SyntaxNode): Map<string, ImportTarget> {
	...
	for (let i = 0; i < root.namedChildCount; i++) {
		...
		if (modSpec.type === "relative_import") {
			isRelative = true;
			const prefix = modSpec.namedChildren.find(
				(c) => c.type === "import_prefix",
			);
			relativeDepth = prefix ? prefix.text.length : 1;
			const dotted = modSpec.namedChildren.find(
 ...
 * sees an isolated class. This walker deliberately leaves this gap; full
 ...
 * detectable; full path resolution belongs in a dedicated resolver.
 */
interface ClassInfo {
	name: string;
	body: SyntaxNode;
...
function readMetaInner(classBody: SyntaxNode): MetaInfo {
	...
	for (let i = 0; i < classBody.namedChildCount; i++) {
		...
		for (let j = 0; j < inner.namedChildCount; j++) {
			...
			else if (lhs.text === "verbose_name_plural")
				meta.verboseNamePlural = value;
		}
	}
	return meta;
}
