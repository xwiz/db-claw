...
`,
        ...
        const entityFrags = r.fragments.filter((f) => f.canonical.kind === "entity");
...
`,
        ...
        const total = r.fragments.find(
            (f) => f.canonical.kind === "field" && f.canonical.field === "order.total",
        );
...
`,
        ...
        const f = r.fragments.find(
            (f) => f.canonical.kind === "field" && f.canonical.field === "auth_users.email",
        );
...
`,
        ...
        const r = await scanDjangoModels(tmp);
        const ratings = r.fragments.filter((f) => f.canonical.kind === "enum_value");
...
`,
        ...
        expect(m.get("active")).toEqual({ name: "user.status", raw: "active" });
        expect(m.get("inactive")).toEqual({ name: "user.status", raw: "inactive" });
        expect(m.get("pending review")).toEqual({ name: "user.status", raw: "pending" });
    });
...
`,
    ...
    it("does not emit fragments for unknown class refs", async () => {
        ...
        // Cross-file resolution lands in v1.0.
        await write(
            "x/models.py",
            `from django.db import models
class User(models.Model):
...
`,
    ...
    it("resolves cross-file TextChoices via the global registry", async () => {
        ...
        // import-resolution lands in v1.0.5.)
        await write(
